...
| Path | What it is |
|---|---|
| `convertion/` | The conversion kit — Bicep infrastructure (`infra/`, `enterprise/`), provisioning, agent charters and verifier rules, MCP server, 14 read-only OpenAPI integrations (`integrations/openapi/`: OneTrust, SecurityScorecard, Defender Graph, Entra IAM Graph, Jira Cloud, Jira Assets CMDB, SharePoint Graph, Confluence, Exchange/Teams/M365 Graph, Azure DevOps, OSINT proxy, IAF), 16 Logic Apps workflows + `pipelines.json`, orchestrator and advisor, human-approval governance, team model and operations — plus the delivery layer (`convertion/REQUIREMENTS.md`): supplier/service SharePoint storage pipelines, Global CISO deck, evidence/SOC/pentest analyzers, read-only enterprise access, egress guardrails, model routing, template change control |
