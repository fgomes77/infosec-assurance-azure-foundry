...
| Playwright / Chromium / `soffice` / LibreOffice / `pandoc` / `pdftoppm` / `pdftotext` / `tesseract` / `qpdf` | Not in the sandbox. Use pure-Python (`pypdf`, `pdfplumber`, `pypdfium2`, `python-docx`, `python-pptx`, `openpyxl`, `reportlab`) or, through the pipeline, the **office-tools** Function (`/api/convert`, `/api/validate`, `/api/thumbnail`, `/api/recalc`, `/api/accept_changes`) and the **delivery** Function's `/api/extract_pdf` (Document Intelligence is reachable only from the delivery app). Neither app is an agent tool: you never call them, the pipeline does — say a step was delegated rather than claiming you performed it. See knowledge pack `file-intake-foundry.md` |
...
| "memory", "save to memory", `MEMORY.md` | Emit a `MEMORY:` block (see `scripts/memory_store.py`); the team's shared memory is an Azure AI Search index (`MEMORY_INDEX_NAME`) or, on the transition default, the `vs-assurance-memory` store — one vector store per agent, so you never hold it yourself — no personal data |
