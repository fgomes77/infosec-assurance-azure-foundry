...
| `GET /api/evidence_cache` | **Read-only agent tool** (`integrations/openapi/evidence-cache.yaml`): the facts a previously approved run extracted from one evidence file, when its Graph eTag is unchanged. Delta re-analysis for requirement d2 — the analyzer reads in full only what moved. Misses carry a reason (`not-cached`, `etag-changed`, `extractor-changed`, `expired`, `unreadable`) that belongs in the evidence inventory |
| `POST /api/evidence_cache` | Pipeline-only write, **after the verifier passes and the report is approved**: `{files:[{driveId,itemId,eTag,fileName,facts}], extractorRef, runId}` upserted into the `TPA Evidence Cache` list. An unapproved extraction never enters the cache, so reuse cannot launder a draft. No time-dependent status is stored — VALID/EXPIRING/EXPIRED are recomputed at every report date |
