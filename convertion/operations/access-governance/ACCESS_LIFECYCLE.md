...
Companion to `../../team/least-privilege/IDENTITY_RBAC.md`. Controls:
...
| 3 | Joiner completes onboarding: reads `governance/*.md`, `sharepoint/README.md`, `team/least-privilege/THREADS_MEMORY.md`; runs the MCP server locally with own `az login`; produces one tier-2 report end-to-end and reviews one peer's tier-1 draft | joiner + a peer | Attestation form `Governance/Onboarding/{upn}.md` (date, systems exercised, peer) | EU AI Act Art. 26(2) competence of the natural person exercising oversight |
...
| 5 | Owner adds to Teams channels `{teams:infosec-assurance-platform}` and `{teams:infosec-assurance-approvals}`, GitHub team `infosec-assurance-readers` (Read) | owner | — | Read-only repo access |
...
| Case | Action | Who |
|---|---|---|
| User becomes deputy approver | Owner nominates, line manager confirms; Entra IAM team adds to `sg-infosec-foundry-platform-approvers` and sets PIM eligibility in `sg-infosec-foundry-breakglass`; GitHub `Write` for PR review; update `team/least-privilege/entra/groups.json` and `approvals/routing.json` (`principals.deputy`) via a PR reviewed by the owner | owner, line manager, Entra IAM team |
...
| 4 | Threads: re-tag active engagement threads (`metadata.owner` → new engagement owner) or delete them; delete the leaver's `personal-working` threads; shared memory notes stay (team facts, not personal data) — nothing personal to purge by design (`THREADS_MEMORY.md` §3) | owner | Thread list before/after |
...
ledger (`team/least-privilege/TEAM_MODEL.md` §4) is re-confirmed row by
