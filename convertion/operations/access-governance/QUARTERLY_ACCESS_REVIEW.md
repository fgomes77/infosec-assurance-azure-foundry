...
| 12 | Conversations older than 90 days with approved deliverables | conversation list (`conversations.json`; classic threads retire 2027-03-31) | owner | owner | Deleted or justified (open engagement / audit hold) | Delete |
...
| 16 | **Agent identities (Entra Agent ID, finding C7)** | project JSON `agentIdentityId` + Entra admin center > Agent ID | owner | `{group:iam-admins}` | Every published-agent identity matches `../../team/ACCESS_REGISTER.md` "Entra Agent ID principals"; no unregistered blueprint; no agent identity holds a write role | De-register / revoke |
| 17 | **Portal drift** | connections, deployments, RAI policies, capability host and network injection compared with the templates per `../../enterprise/PORTAL_CONFIGURATION.md` §4; policy compliance export `az policy state list -g rg-infosec-foundry` | owner | deputy | No difference from the deployed templates; policy state compliant | Revert by redeploying `infra/main.bicep` (never fix in the portal) — RUNBOOK FM-34 |
| 18 | **Platform currency** | model retirement schedule vs the pinned versions, EU Data Zone availability (incl. Claude), preview → GA transitions used by the platform (A2A, SharePoint grounding tool, Memory, Foundry IQ); `python3 ../../enterprise/upgrade/check_model_lifecycle.py --dry-run --horizon 180` | owner | deputy | 0 errors; every open finding has a change ticket (`../../enterprise/UPDATE_AND_UPGRADE_REVIEW_POLICY.md` R2) | Open the ticket |
