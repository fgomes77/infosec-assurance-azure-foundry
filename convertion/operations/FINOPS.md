...
> Standing audit of every efficiency lever, what changed and what is
> deliberately left alone: `OPTIMIZATION_REVIEW.md`.
...
| C6 | Log Analytics + App Insights (workspace-based) | `{baseName}-logs`, `{baseName}-appi` | GB ingested/day; **interactive 90 d + archive to 365 d** (`MONITORING.md` §2) | €120 → €250 | **applied:** the retention split — the evidence horizon stays 365 days, but eleven of those twelve months sit in the archive tier at roughly a tenth of the price. Still open: Basic-logs tier for `FunctionAppLogs`. Sampling of `AppTraces` is possible (never of `AppDependencies` — the egress detector needs them) and the token queries are already `ItemCount`-weighted so their totals survive it | `logRetentionDays`, `logInteractiveRetentionDays` |
