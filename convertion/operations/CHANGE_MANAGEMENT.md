...
| **Access** (groups, roles, Sites.Selected, CA, PIM) | `rbac.bicep`, `least-privilege/entra/groups.json`, access packages | line manager for privileged groups; owner for user groups | `provision_identity.sh --plan`; `what-if` | `provision_identity.sh --apply` / `rbac.bicep` | previous assignment set; `access_snapshot.sh` before/after |
...
| 6 | CI (`.github/workflows/`, see delta D-OPS-G1 in §10): the same gates as step 3 on a clean runner + `access-review.sh --quick` in verify-only mode; must be green | checks |
...
|---|---|---|
| D-OPS-G1 | `.github/workflows/ci.yml` (new, repo root) | job `gates` on `pull_request`: `python3 convertion/scripts/convert_skills.py && python3 convertion/scripts/verify_conversion.py && (cd convertion && ./deploy.sh --dry-run) && for f in $(git ls-files 'convertion/**/*.json'); do python3 -m json.tool "$f" >/dev/null; done && python3 -m py_compile convertion/scripts/*.py convertion/functions/delivery/function_app.py convertion/mcp-server/server.py && bicep build convertion/infra/main.bicep && bicep build convertion/team/rbac.bicep && bicep build convertion/operations/alerts.bicep`; job `deploy` on `push` to `main`, `environment: production`, `permissions: id-token: write`, `azure/login` with the federated credential of `{app:infosec-foundry-deployer}`, then `cd convertion && ./deploy.sh` |
