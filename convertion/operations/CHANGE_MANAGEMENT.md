...
## 10. Shared deltas raised by this process — all applied

The rows are kept as the record of what each change was and where it
landed; the targets below are live files, not proposals.

| Id | Target | Literal text |
|---|---|---|
| D-OPS-G1 | `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`, `.github/workflows/nightly-drift.yml` | **Applied.** `ci.yml` is the gate job only and never authenticates to anything: convert_skills → verify_conversion → verify_kit → `convertion/ci/syntax_check.sh` → `[4b] EU residency` (`ci/tests/test_residency.py`) → `[4c] build_self_knowledge.py --check` → `[4d]` the pytest suites → `convertion/infra/validate.sh` → `run_evals.py --dry-run` + `--emit-plans` → `check_model_lifecycle.py` + `learning_loop.py` → `./deploy.sh --dry-run`; plus the parallel `secret-scan` job (gitleaks over tree and history). The `deploy` job is **no longer part of ci.yml**: it is `.github/workflows/deploy.yml`, triggered by `workflow_run` on a *successful* CI run on `main`, checking out `workflow_run.head_sha` so it deploys the commit CI actually verified, gated by `environment: production`, authenticating with the OIDC federated credential of `{app:infosec-foundry-deployer}` (`permissions: id-token: write`), asserting EU residency **offline before the credential is minted and `--live` after login**, then running `deploy.sh` with `STRICT_RUNTIME=1` / `LIFECYCLE_STRICT=1` and publishing `build/agent-versions.json`. `.github/workflows/nightly-drift.yml` is the third file: nightly `verify_deployment.py --json` + `team/access-review.sh` on a read-only federated identity, no approval, 90-day redacted artefact. Azure DevOps equivalent of all three: `convertion/ci/azure-pipelines.yml`. Documentation: `convertion/ci/README.md`. |
| D-OPS-G2 | `.github/CODEOWNERS`, `.github/PULL_REQUEST_TEMPLATE.md`, `.pre-commit-config.yaml` | **Applied.** The file lives at **`.github/CODEOWNERS`**, not at the repository root: GitHub resolves `.github/CODEOWNERS` ahead of a root copy, so a root `CODEOWNERS` beside it would be silently ignored — a trap worth naming, because the ignored file would still *look* like the control. Content: `convertion/ @{github:francisco-gomes}`, and `convertion/team/ convertion/operations/ convertion/governance/ @{github:francisco-gomes} @{github:deputy-approver}` — owner **plus** deputy on anything that changes privilege, policy or what "green" means. Branch protection: require review from Code Owners, dismiss stale reviews, no self-approval. `.github/PULL_REQUEST_TEMPLATE.md` is the implementation of the §3/§4 review checklist (it becomes the PR body, so the checklist is filled where the reviewer reads it). `.pre-commit-config.yaml` is the **local, non-authoritative** mirror of the gates — hygiene hooks, gitleaks, and `language: system` hooks calling `syntax_check.sh`, `test_residency.py`, `verify_kit.py` and (pre-push) `deploy.sh --dry-run`. Local only: CI remains the authority, and `--no-verify` cannot weaken it. |
