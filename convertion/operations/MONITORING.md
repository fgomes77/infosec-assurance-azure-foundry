...
|---|---|---|---|
| `{baseName}-logs` | `retentionInDays` (interactive) | **90** — `logInteractiveRetentionDays` | The queryable window. Every shipped query and alert looks back at most 30 days, so 90 covers a full quarter of incident review; paying interactive rates for the other 275 days bought nothing |
| `AppDependencies`, `AppRequests`, `AppExceptions`, `AppTraces` | `totalRetentionInDays` (interactive + archive) | **365** — `logRetentionDays` | DORA Art. 28 evidence ≥ 1 year (`DATA_PROTECTION_GUARDRAILS.md` §4); approval and egress evidence. Archive costs roughly a tenth of interactive storage and is restored or searched when an audit or investigation needs it — the evidence horizon is unchanged, only its price is |
| `{baseName}-logs` | `workspaceCapping.dailyQuotaGb` | **-1 (uncapped)** — `logDailyQuotaGb` | Deliberate: a cap does not slow ingestion, it DROPS telemetry for the rest of the day, and the dropped items would include the egress detector's `AppDependencies`. Cost is bounded by the retention split above and the budget alert, never by discarding security evidence |
...
| D-OPS-B3 | `infra/main.bicep` | `logAnalytics.properties` | **APPLIED** — superseded by the retention split: `retentionInDays: logInteractiveRetentionDays` (90) on the workspace plus `totalRetentionInDays: logRetentionDays` (365) per evidence table, which keeps the DORA Art. 28 horizon at archive prices |
