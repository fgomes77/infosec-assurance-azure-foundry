...
| M5 | Retention and cleanup run — the ISMS retention control (`RETENTION_AND_CLEANUP.md` §4) | `python3 ../scripts/cleanup_foundry.py --dry-run` is the **offline rehearsal** and needs no Azure session; then `python3 ../scripts/cleanup_foundry.py --all` as a **read-only listing** filed as evidence; then, and only after the owner has approved that listing, the same command with `--apply`; expired team notes additionally need `--memory --apply --approved-by "{upn:francisco.gomes}"`. Every horizon comes from `../setup/.env` (`RETENTION_CONVERSATION_APPROVED_DAYS`, `RETENTION_CONVERSATION_IDLE_DAYS`, `RETENTION_ORPHAN_FILE_DAYS`, `RETENTION_ORPHAN_STORE_DAYS`, `RETENTION_MEMORY_MONTHS`) and `vs-assurance-memory` / `vs-assurance-combined` are never deleted | console output of every step filed as `Governance/Operations/{yyyy}-{mm}/retention-run.md`; protected stores `vs-assurance-memory` / `vs-assurance-combined` still present (check C2) |
