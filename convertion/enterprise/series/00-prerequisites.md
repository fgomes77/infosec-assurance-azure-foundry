...
| P5 | GitHub (or Azure DevOps) repository with branch protection, CODEOWNERS = deputy for owner PRs, `production` environment approval (`operations/CHANGE_MANAGEMENT.md` §2) and an OIDC federated deploy identity `{app:infosec-foundry-deployer}` | no client secrets | repo settings screenshot |
