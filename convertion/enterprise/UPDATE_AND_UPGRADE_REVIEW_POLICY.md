...
| D-UP-5 | `governance/MODEL_ROUTING.md` after the tier table | add "**Tool-support rule (R4 of `enterprise/UPDATE_AND_UPGRADE_REVIEW_POLICY.md`):** a tier model must support every tool in the tier's toolset per https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/limits-quotas-regions#tool-support-by-region-and-model on the day of the change; `o3-mini` fails this for the advisory toolset (no OpenAPI/MCP/Azure AI Search/SharePoint/Web Search) — the reasoning default must be re-selected before production." |
...
| D-UP-8 | `.github/workflows/ci.yml` (delta D-OPS-G1) job `gates` | append `python3 convertion/enterprise/upgrade/check_model_lifecycle.py --dry-run --horizon 180 && python3 convertion/enterprise/memory/learning_loop.py --dry-run --out /tmp/ll && bicep build convertion/enterprise/upgrade/model-deployment-policy.bicep` |
| D-UP-9 | `infra/monitoring.bicep` / `infra/kql/agent-drift.kql` | alert `agent_version_not_in_manifest`: a run whose `gen_ai.agent.version` (or agent id) is not listed in the deployed `build/manifest.json` versions — portal edit or unpromoted version serving traffic |
