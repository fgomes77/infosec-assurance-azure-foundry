...
| C8 | **Workflows / pipelines** | `workflows/*.json`, `pipelines.json`, `approval-policy.json`, `FOUNDRY_API_VERSION` | Medium; High if an approval step, expiry or `approvalKind` changes | JSON parse; `build_logicapps.py --dry-run`; one run to the verifier in test (never approved); approval expiry unchanged or justified; security review | `HUMAN_APPROVAL.md` Layer 3 table updated in the same PR |
...
| 8 | Bicep api-versions for `Microsoft.CognitiveServices` (kit: **`2025-06-01` stable everywhere**; `2025-09-01`, `2025-12-01` exist) | https://learn.microsoft.com/en-us/azure/templates/microsoft.cognitiveservices/accounts/projects (2026-07-20) | done for the preview→`2025-06-01` move (compiled ARM identical apart from the version string); the next bump moves when `what-if` shows no diff (patch) |
