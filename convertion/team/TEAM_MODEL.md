...
|---|---|
| Built-in role GUIDs in `rbac.bicep` | `az role definition list --query "[].{n:roleName,id:name}"` — note: `rbac.bicep` (`53ca6127-db72-4b80-b1b0-d745d6d5456d`) and `least-privilege/infra/rbac.bicep` (`…d229d5fc3ae7`) disagree on the `Azure AI User` id; the published built-in definition is `53ca6127-db72-4b80-b1b0-d745d6d5456d`, so align the least-privilege copy (delta D-T1) and confirm with the command before the first `what-if` |
