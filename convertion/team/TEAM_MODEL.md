...
| `rbac.bicep` + `rbac.parameters.example.json` | Azure role assignments per §7 (PIM eligibilities per §6 — see delta D-T1) |
| `approval-policy.json` | machine-readable §12 for the approval flow behind `approvalWebhookUrl` (delta D-T2 aligns kinds) |
| `custom-role.agent-consumer.json` | preventive control of §7.1 |
| `least-privilege/entra/groups.json` | group definitions, owners, review cadence (§6; delta D-T3 re-keys to the seven groups) |
...
| F7 | Incident (egress alert, verifier FAIL spike, injection, credential exposure, outage) | any user reports / stops own runs; owner leads; deputy if owner absent | owner PIM / deputy break-glass | disable connection, workflow, agent tool or secret without approval; change only with approval |
...
| F9 | Monthly evaluation + cost review (`../ARCHITECTURE.md` metrics) | owner; users consulted | owner standing `Log Analytics Reader` | users need no monitoring role (§5) |
...
| F12 | Support | requester → peer (L1) → owner (L2) → landing-zone / Microsoft (L3) | — | Teams channel `{teams:infosec-assurance-platform}` |
...
Key Vault `{baseName}-kv` is added by delta D-B2). Standing = permanent;
...
The full matrix — one Accountable per row, the five people named
individually, deputy overlay, external parties — is `RACI.md`. Summary:
the owner is A for every lifecycle activity except the review of his own
privileged access and break-glass approval (line manager) and the GDPR
Art. 33 notification path (DPO).
...
|---|---|
| Built-in role GUIDs in `rbac.bicep` | `az role definition list --query "[].{n:roleName,id:name}"` — note: `rbac.bicep` and `least-privilege/infra/rbac.bicep` currently disagree on the `Azure AI User` id; the built-in role is `53ca6127-db72-4b80-b1b0-d229d5fc3ae7` (delta D-T1) |
...
| Bicep compile | `bicep build` was not run in the authoring environment (no CLI); run `az deployment group what-if` on `rbac.bicep` |

## 20. Shared deltas (literal text — applied by the kit maintainer, not here)

Deltas to files outside `team/` and `operations/` and to pre-existing
files in this folder. Ids are stable so `ACCESS_REGISTER.md` and PRs can
cite them.
...
| D-OP1 | `operations/access-governance/{ACCESS_LIFECYCLE,QUARTERLY_ACCESS_REVIEW,BREAK_GLASS}.md` | every `team/least-privilege/…` reference | point to `team/TEAM_MODEL.md` (§14, §15, §12.3) and rename `-platform-approvers` → `-senior-approvers`, `-platform-admins` → `-admin-pim`, `-auditors` → `-readers`, `tier-1/2/3` → `Tier B/A/C`; QUARTERLY item 5: `exactly three Sites.Selected grants (Function MI write, project MI read, Logic App MI read)`; QUARTERLY item 13: `10 % of Tier A approvals + 100 % of Tier B fallbacks` |
