...
orchestrator) it reports, exactly like verify_sync.py did:
    ...
    IDENTICAL        instructions hash, tools, model all match
    DIFFERENT        one or more of: instructions (sha256 after stripping
                     the deploy-time FILE MAP / advisory addendum blocks),
                     tool set (file_search / code_interpreter / openapi /
                     mcp / bing / connected agents vs manifest+registry),
                     model deployment (tier -> deployment name), an OpenAPI
...
def expected_agents() -> dict[str, dict]:
    ...
    for a in mf["agents"]:
        ...
        out[a["name"]] = {"sha": sha(strip_deploy_blocks(text)),
                          "tools": set(a["tools"])
                          | ({"connected_agent"} if a["router_targets"] else set()),
                          "model": CHAT_MODEL}
    ...
    out[co.ADVISOR] = {"sha": sha(strip_deploy_blocks(co.advisor_instructions())),
                       "tools": {"file_search", "bing_grounding"},
    ...
    out[co.ORCHESTRATOR] = {"sha": sha(strip_deploy_blocks(co.orchestrator_instructions())),
                            "tools": {"bing_grounding", "connected_agent"},
                            "model": REASONING_MODEL}
    ...
    for name in REGISTRY.get("advisory_read_only_toolset", {}).get("agents", []):
        if name in out:
            out[name]["tools"] |= {"code_interpreter", "file_search"}
    return out
...
def main() -> int:
    ...
    live = {a.name: a for a in ac.list_agents()}
