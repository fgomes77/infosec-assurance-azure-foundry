...
**Why the research ledger exists, and where it lives.** The other heavy
recurring cost is research: every assessment asked the same sources the same
questions about the same supplier — its trust centre, its sub-processors, the
CVEs in its stack, its certificates in the registry — and paid the latency,
the tokens and the outbound call again, while nothing recorded what had been
asked or why. The ledger keeps each observation with its date, its citation
and **the action that caused it**, and the agents read it before searching: a
record inside its freshness window is quoted with its observation date, and
only a stale one is fetched again. Freshness is per source (KEV and EPSS one
day, NVD seven, a trust centre fourteen, the ESAs thirty, EUR-Lex ninety,
ISO/NIST one hundred and eighty), and nothing is reused past
`RESEARCH_LEDGER_MAX_AGE_DAYS`.

The same records are written, in human-readable form, to the supplier's own
knowledge file:
...
One table, newest first: observed date, service, source, the question, what
came back, the citation, the originating action and the run. It is
regenerated by the Function on every store — never edited by hand — and it is
what a reviewer, an auditor or the next analyst opens to see what the platform
knows about a supplier and how it came to know it. It carries the same
sensitivity as the reports beside it: it reveals which suppliers are under
assessment, so it is Euronext Confidential and never goes to a web-facing
tool.

Two things are deliberately NOT reusable from it: a **time-dependent status**
(valid / in force / exploited — recomputed from the observation against each
report date, exactly as in the evidence cache) and anything carrying an
internal marker, which the Function refuses to store at all, so the ledger
